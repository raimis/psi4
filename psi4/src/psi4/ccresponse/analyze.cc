...
void analyze(const char *pert, int irrep, double omega)
{
  ...
  auto printer = std::shared_ptr<PsiOutStream>(new PsiOutStream(lbl, std::ostream::app));
  //ffile(&efile, lbl, 1);
  ...
  auto printer2 = std::shared_ptr<PsiOutStream>(new PsiOutStream(lbl, std::ostream::app));
  //ffile(&efile, lbl, 1);
  amp_array = init_array(num_div);
